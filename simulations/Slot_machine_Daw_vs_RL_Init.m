...
Cal_mu_pre=@(Mu_post_old,lambda,theta)lambda.*Mu_post_old+(1-lambda).*theta;
Cal_sigma_pre=@(Sigma_post,lambda,Sigma_d)lambda.^2.*Sigma_post.^2+Sigma_d.^2;
...
for i=1:trialNum %模拟强化学习
    ...
    if Choice==1
        ...
        mu_a=Cal_mu_pre(mu_a_post,lambda,theta);
        sigma_a=Cal_sigma_pre(sigma_a_post,lambda,Sigma_d);
        ...
        mu_b=Cal_mu_pre(mu_b_post,lambda,theta);
        sigma_b=Cal_sigma_pre(sigma_b_post,lambda,Sigma_d);
    end
    MU_A_Box(i)=mu_a;
    MU_B_Box(i)=mu_b;
end
...
for k=1:tot
    ...
    x0 = [rand rand]; %起始点设置
    [paramsEst, minuslli, ~] = ...
        fminsearchbnd(@(params)RL_Init(params,RA_Box,RB_Box,trialNum,N,Choice_Box), x0, LB, UB,options);%并利用fminsearchbnd函数获得最大似然的值
    alphaBox(k) = paramsEst(1);betaBox(k) = paramsEst(2); LLBox(k)= minuslli;
end
...
Choice_Box=zeros(size(RA_Box));
mu_a=50;mu_b=50;%新的起始点
for i=1:trialNum %模拟强化学习
    Pca=P_choosei_new(mu_a,mu_b);
    Choice=binornd(1,Pca);Choice_Box(i)=Choice;
    if Choice==1
        rQ=RA_Box(i);
        PE_Box2(i)=rQ-mu_a;
